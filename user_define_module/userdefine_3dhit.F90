  ...
  subroutine udf_src
    ...
    if(lforce .and. trim(flowtype)=='hittgf') then
      ...
      do i=0,im
        ...
        yy = x(i,j,k,2)
        zz = x(i,j,k,3)
        forcep(i,j,k,1) = Am * ( cos(   k0*xx) * sin( 2*k0*yy) * sin(-3*k0*zz) + &
                                 cos( 2*k0*xx) * sin(-3*k0*yy) * sin(   k0*zz) + &
                                 cos(-3*k0*xx) * sin(   k0*yy) * sin( 2*k0*zz) )
        forcep(i,j,k,2) = Am * ( sin(   k0*xx) * cos( 2*k0*yy) * sin(-3*k0*zz) + &
                                 sin( 2*k0*xx) * cos(-3*k0*yy) * sin(   k0*zz) + &
                                 sin(-3*k0*xx) * cos(   k0*yy) * sin( 2*k0*zz) )
        forcep(i,j,k,3) = Am * ( sin(   k0*xx) * sin( 2*k0*yy) * cos(-3*k0*zz) + &
                                 sin( 2*k0*xx) * sin(-3*k0*yy) * cos(   k0*zz) + &
                                 sin(-3*k0*xx) * sin(   k0*yy) * cos( 2*k0*zz) )
      enddo
      enddo
      enddo
      !
