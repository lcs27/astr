...
module userdefine
  ...
  implicit none
  !
  real(8) :: flamethickness,hsource,equivalence_ratio=1d0
  ...
  subroutine udf_flowinit
    ...
    real(8) :: pthick,theter,radii,var1
    !
    tmpr=300.d0
    xloc=4.d0*xmax/5.d0
    ...
    do i=0,im
      ...
      if(var1<=1.d-16) then
        ...
        theter=asin(yc/sqrt(xc**2+yc**2))
      endif
      !
      radii=pthick+0.05d0*pthick*sin(theter*30.d0)
      !
      prgvar=1.d0*exp(-0.5d0*(xc**2+yc**2)/radii**2)
      !
      ! prgvar=1.d0*exp(-0.5d0*(xc**2+yc**2)/pthick**2) + &
      !        1.d0*exp(-0.5d0*((xc-0.02d0)**2+yc**2)/pthick**2)
      !
      spc(i,j,k,:)=specr(:)
      !
      vel(i,j,k,1)=uinf
